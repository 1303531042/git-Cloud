...
public class ThingsModelServiceImpl implements IThingsModelService 
{
    ...
    public List<ThingsModel> selectThingsModelList(ThingsModel thingsModel) {
        return thingsModelMapper.selectVoList(new QueryWrapper<ThingsModel>().setEntity(thingsModel));
    }
    @Override
    public ThingsModel selectThingsModelOne(ThingsModel thingsModel) {
        return thingsModelMapper.selectVoOne(new QueryWrapper<ThingsModel>().setEntity(thingsModel));
    }
...
//            thingsModel.setTenantName(user.getUserName());
            ...
            int result = thingsModelMapper.insert(thingsModel);
            // 更新redis缓存
...
//            thingsModel.setTenantName(user.getUserName());
    ...
    {
        // 物模型标识符不能重复 TODO 重复查询待优化
        List<ThingsModel> list=thingsModelMapper.selectVoList(new QueryWrapper<ThingsModel>().eq("model_id",thingsModel.getProductId()));
...
//        // redis获取物模型
    ...
    @Override
    public List<ThingsModel> getThingsModleToEchartsByProductId(Long productId) {
        return thingsModelMapper.selectList(new QueryWrapper<ThingsModel>()
    ...
    private String setCacheThingsModelByProductId(Long productId){
        // 数据库查询物模型集合
        List<ThingsModel> thingsModels = thingsModelMapper.selectVoList(new QueryWrapper<ThingsModel>().eq("product_id", productId));
        // 转换为物模型
        ...
        for(int i=0;i<thingsModels.size();i++){
            if(thingsModels.get(i).getType()==1){
                ...
                propertyDto.setIsEcharts(thingsModels.get(i).getIsEcharts());
