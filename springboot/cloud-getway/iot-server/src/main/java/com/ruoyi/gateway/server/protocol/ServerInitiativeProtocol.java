...
import com.ruoyi.gateway.client.ClientProxySync;
...
import com.ruoyi.gateway.server.ServerTimeoutProtocolManager;
...
import com.ruoyi.gateway.server.manager.DevicePipelineManager;
...
public abstract class ServerInitiativeProtocol<M extends ServerMessage> extends
     ...
     * 默认用报文头的{@link Message.MessageHead#getMessageId()}作为key
     * @return
     */
    ...
    public void request() throws ProtocolException {
        try {
            ...
            if(channel == null) {
                throw new IllegalStateException("无此设备或设备断线["+getEquipCode()+"]");
            }
    ...
    protected ExecStatus writeAndFlush(SocketServerComponent component) throws InterruptedException {
        ...
        ChannelFuture future = (ChannelFuture) component.writeAndFlush(getEquipCode(), this).get();

        if(!future.await(getTimeout(), TimeUnit.MILLISECONDS)) {
            this.reason = "请求超时";
            setExecStatus(result = ExecStatus.timeout);
        }

        if(!future.isSuccess()) {
            this.setExecStatus(result = ExecStatus.fail);
            this.reason = future.cause() != null ? future.cause().getMessage() : "请求失败";
    ...
    /**
     * 这里的移除将比超时管理器早
     * @see ServerTimeoutProtocolManager#(ProtocolTimeoutStorage) 协议超时管理
     */
