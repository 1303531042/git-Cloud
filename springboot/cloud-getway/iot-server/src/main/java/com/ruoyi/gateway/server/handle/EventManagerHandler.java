package com.ruoyi.gateway.server.handle;

import com.ruoyi.gateway.CoreConst;
import com.ruoyi.gateway.DeviceManager;
...
import com.ruoyi.gateway.codec.filter.RegisterParams;
...
import com.ruoyi.gateway.server.ServerComponentFactory;
        ...
        if(ctx.channel().isActive()) {
            ctx.channel().close().addListener((ChannelFutureListener) future -> {
                ...
                if(future.isSuccess()){
                    ...
                    logger.warn("客户端超时下线({}) {} - 客户端编号: {} - 客户端地址: {} - 状态: 下线失败"
                            , component.getName(), desc, equipCode, target, future.cause());
                }
            });
        }
        ...
        try {
            ...
            if(equipCode != null && logger.isWarnEnabled()) {
                ...
                if(aBoolean == null) {
                    ...
                    if(logger.isWarnEnabled()) {
                        logger.warn("客户端断线({}) 客户端重复导致被剔除下线 - 客户端编号: {} - 客户端地址: {}", serverComponent.getName(), equipCode, remoteAddress);
                    }
                }
