package com.ruoyi.gateway.server.manager;
...
public class DevicePipelineManager extends DefaultChannelGroup implements DeviceManager {
    ...
    public Optional<ChannelFuture> writeAndFlush(String equipCode, Object msg, Object... args) {
        if(msg instanceof Protocol) {
            ...
            if(null == channel) {
                throw new IllegalStateException("无此设备或设备断线["+equipCode+"]");
            } else if(!channel.isActive()) { // 设备已经取消注册, 删除设备
                remove(equipCode);
                logger.warn("设备在线管理({}) 设备断线 - 设备编号: {} - 已连接：{} - 已注册：{}", this.name(), equipCode, size(), useSize());
                return Optional.of(channel.newFailedFuture(new ProtocolException("设备断线["+equipCode+"]")));
            }
    ...
    public List<String> getChannelCodes() {
        return clientChannels.entrySet().stream().map(Map.Entry::getKey).collect(Collectors.toList());
    }
}
