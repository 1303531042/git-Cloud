...
public abstract class ClientInitiativeProtocol<C extends ClientMessage> extends ClientSocketProtocol<C> implements ProtocolPreservable {
    ...
    protected void requestMessageHandle(SocketClient client) {
        if(client.getChannel() != null) {
            requestMessage().setChannelId(client.getChannel().id().asShortText());
        }
    }
    ...
    /**
     *  请求指定的远程主机
     * @see SocketClientComponent#createNewClientAndConnect(ClientConnectProperties)
     * @param host 远程主机
     * @param port 远程端口
     */
    ...
    /**
     * 设置异步超时时间
     * @see AbstractProtocolTimeoutManager 超时报文管理
     * @param timeout 超时时间
     * @return
     */
