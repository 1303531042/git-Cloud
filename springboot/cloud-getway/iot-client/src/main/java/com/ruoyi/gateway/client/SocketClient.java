...
import com.ruoyi.gateway.client.codec.ClientProtocolEncoder;
import com.ruoyi.gateway.client.component.SocketClientComponent;
import com.ruoyi.gateway.client.handle.ClientServiceHandler;
...
import com.ruoyi.gateway.event.ClientStatus;
import com.ruoyi.gateway.event.ClientStatusEvent;
...
public abstract class SocketClient implements IotClient<NioEventLoopGroup> {
    ...
    /**
     * 断开连接
     * @param remove true.直接断开并移除客户端  false. 直接断开
     */
    public ChannelFuture disconnect(boolean remove) {
        this.channel.attr(CLIENT_CLOSED_DISCONNECT).set(true);
        return this.channel.disconnect().addListener(future -> {
            if(future.isSuccess()) {
                ...
                // 断开失败
                this.channel.attr(CLIENT_CLOSED_DISCONNECT).set(null);
            ...
            try {
                ...
                return connect((arg) -> {
                    ...
                    if(future.isSuccess()) {
                        ...
                        logger.error("客户端({}) 写报文重连失败 - 远程主机 {}:{} - 客户端标识：{}"
                                , getClientComponent().getName(), this.getHost(), this.getPort(), getConfig().connectKey(), future.cause());
                    }
                    // todo 回调函数和监听函数合并异常问题
